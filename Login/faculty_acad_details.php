...
<html>
  <head>
    ...
    {
    var letters = /^[A-Z.a-z]+$/;
    if (h_degree.value.match(letters))
   {
    if ((h_degree.value).length > 15)
   {
    document.getElementById('error1').innerHTML = "Max Size Is 15!";
   }
   else
   {
   ...
   {    
   var letters = /^[A-Z a-z]+$/;
   if (degree_field.value.match(letters))
   {
    if ((degree_field.value).length > 15)
   {
    document.getElementById('error2').innerHTML = "Max Size Is 15!";
   }
   else
   {
 ...
 <body class="skin-blue">
      ...
      <div class="content-wrapper">
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li class="active">Faculty Academic Details</li>
          </ol>
        </section>
        <section>
          <center>
    ...
    <fieldset>
    <legend><h1>Faculty Academic Details</h1></legend>
    <table cellpadding="7px">
    <form name="faculty_acad_details" action="faculty_acad_details_conn.php" method="POST">

      <tr><td><label>Discipline ID :</label></td>
      <td><select name="discipline_id" id="discipline_id"" required="required" onchange="getBranchId(this.value),setDiscipline(this.value)">
      ...
      <tr><td><label>Branch Name :</label></td>
        <td><select name="branch_id" id="branch_id" required="required" onchange="setBranch(this.value),selectFaculty(this.value)"> 
                <option value = "">Select Branch</option>
                </select></td></tr>

      <tr><td><label>Faculty ID :</label></td>
        <td><select name="faculty_id" id="faculty_id" required="required" onchange="selectSubject(this.value)" >    
      ...
      <td><input type="varchar" size="15" name="h_degree" id="h_degree" required="required"
      onblur="validateForm()"/>
      <span id="error1"></span></td>
      </tr>


      <tr><td><label>Degree Field :</label></td>
      <td><input type="text" size="15" name="degree_field" id="degree_field" required="required" 
      onblur="validateForm1()"/>
      <span id="error2"></span></td></tr>
      <tr><td><label>Experience :</label></td>
      <td><input type="number" size="2" name="experience" id="experience" required="required"
       onblur="validateForm2()"/>
       <span id="error3"></span></td></tr>
     ...
     <script>
        ...
        function selectFaculty(val)
        {
          ...
          data:'discipline_id='+disci+'&branch_id='+hr,
          success: function(data){
          $("#faculty_id").html(data);
          }
          });
        }
        function getBranchId(val)
        {
          $.ajax({
          type: "POST",
          url: "selectbranch.php",
